...
public:
	...
	// (such as "player").
	void assignTag(std::string tag);//sticks string tag on a GameObject
	bool hasTag(std::string tag);//returns a bool that tells if tag has been stuck
	
	// "Dead" game objects will be removed from the scene.
	bool isDead();//tell scene game object should be removed
	void makeDead();//sets this to true
